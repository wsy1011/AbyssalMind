...
以下清单不是论文的原始结论，而是 Abyssal Mind 根据上述证据整理的初筛框架：

1. **规则路径：** 目标船舶在 2025、2030、2035 及以后分别受哪些已生效规则约束？是否为全球规则进一步收紧留出了情景预算？
2. **燃料证明：** 每种候选燃料的 well-to-wake 强度、认证边界、电力来源和供应合同能否核验？
3. **船舶转换：** 现有发动机、燃料舱容、续航和安全系统，支持掺混、改造还是双燃料新造？下一次转换的停航与资本成本是多少？
4. **港口连续性：** 航线上每个关键节点是否都有可用、可扩容且符合强度要求的燃料？单港试点能否支撑整段走廊运营？
5. **经济暴露：** 燃料价差、合规成本、电力和碳价变化分别会把项目推向哪种燃料路径？是否将过渡期和终局期的成本分开建模？
...
作者公开的数据入口：[Zenodo 数据集](https://doi.org/10.5281/zenodo.19872982)。图 1—5 从论文附件提取，保留完整面板与图例，未重绘或修改数据；提取记录及文件校验值保存在 `assets/articles/phased-fuel-transitions/provenance.json`。

原图版权 © The Author(s) 2026，许可为 [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/)。中文说明为 Abyssal Mind 独立解读，不是原图注逐字翻译。
